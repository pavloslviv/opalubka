	...
	function wpex_display_social() {
		...
		$output = '<ul id="social" class="clearfix">';
				...
				foreach ( $wpex_social_links as $social_link ) {
					...
					if ( wpex_get_data($social_link) ) {
					
						$output .= '<li class="'. $social_link .'"><a href="'. wpex_get_data($social_link) .'" title="'. $social_link .'" target="_blank" class="office-tooltip"><img src="'. get_template_directory_uri() .'/images/social'. $social_style .'/'.$social_link.'.png" alt="'. $social_link .'" /></a></li>';
