...
function office_breadcrumbs() {
	...
	if( function_exists('yoast_breadcrumb') ) {
		...
		yoast_breadcrumb('<nav id="breadcrumbs" class="clearfix"><span class="breadcrumbs-title">'. __('Вы находитесь здесь','wpex') .':</span>','</nav>');
		
	} else {

	// Built-in Breadcrumbs
	  $delimiter = '&raquo;';
	  $home = __('Прогрес ДФ','wpex');
	  $before = '<span>';
	  $after = '</span>';
	 
	  if ( !is_home() && !is_front_page() || is_paged() ) {
	 
		echo '<nav id="breadcrumbs"><span class="breadcrumbs-title">'. __('Вы находитесь здесь','wpex') .':</span>';
		...
		//post-type
		  ...
		  if ( get_post_type() == 'post' ) {
			if(wpex_get_data('blog_url')) {
				echo '<a href="' . wpex_get_data('blog_url') . '">' . __('Blog','wpex') . '</a> ' . $delimiter . ' ';
			}
		  ...
		  if ( get_post_type() == 'product' ) {
			  ...
			  if ( $wooshopurl ) {
			  	echo '<a href="' . $wooshopurl . '">'. __('Shop','wpex') .'</a>';
			  }
		  }
		...
		elseif (is_tax( 'portfolio_cats') ) {
		  if(wpex_get_data('portfolio_url')) {
			  echo '<a href="' . wpex_get_data('portfolio_url') . '">' . __('Portfolio','wpex') . '</a> ' . $delimiter . ' ';
		  }
		...
		elseif (is_tax( 'service_cats') ) {
		  if(wpex_get_data('services_url')) {
			  echo '<a href="' . wpex_get_data('services_url') . '">' . __('Services','wpex') . '</a> ' . $delimiter . ' ';
		  }
		  echo $before . $term->name . $after;
		}
		...
		elseif (is_tax( 'staff_departments')) {
			if(wpex_get_data('staff_url')) {
				echo '<a href="' . wpex_get_data('staff_url') . '">' . __('Staff','wpex') . '</a> ' . $delimiter . ' ';
			}
			 echo $before . $term->name . $after;
		}
		...
		elseif (is_tax( 'faqs_cats')) {
			if(wpex_get_data('faqs_url')) {
				echo '<a href="' . wpex_get_data('faqs_url') . '">' . __('FAQs','wpex') . '</a> ' . $delimiter . ' ';
			}
			 echo $before . $term->name . $after;
		}
		...
		if ( get_query_var('paged') ) {
		  ...
		  echo ' ' .$delimiter . __(' Page', 'wpex') . ' ' . get_query_var('paged');
		  if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() );
		}
