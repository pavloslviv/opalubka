    ...
    <header id="page-heading">
		...
		<!-- Portfolio Filter -->
			<ul id="portfolio-cats" class="filter clearfix">
				<li><a href="#all" rel="all" class="active" data-filter="*"><span><?php _e('All', 'wpex'); ?></span></a></li>
				<?php
				foreach ($cats as $cat ) : ?>
                ...
                <?php if ( has_post_thumbnail() ) :  ?>
                    <article class="portfolio-item <?php if($terms) { foreach ($terms as $term) { echo $term->slug .' '; } } ?>">
                        <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
                            <img src="<?php echo aq_resize( wp_get_attachment_url( get_post_thumbnail_id() ), wpex_img('portfolio_entry_width'),  wpex_img('portfolio_entry_height'),  wpex_img('portfolio_entry_crop') ); ?>" alt="<?php echo the_title(); ?>" />
                            <h3><?php the_title(); ?></h3>
                        </a>
